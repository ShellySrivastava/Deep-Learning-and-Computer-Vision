...
def discriminator(x, drop_out):
    ...
    h0 = tf.nn.leaky_relu(tf.matmul(x, w0) + b0)
    hh0 = tf.nn.dropout(h0, rate=drop_out)
    # 2nd hidden layer
    w1 = tf.get_variable('D_w1', [hh0.get_shape()[1], 512], initializer=w_init)
    b1 = tf.get_variable('D_b1', [512], initializer=b_init)
    h1 = tf.nn.leaky_relu(tf.matmul(hh0, w1) + b1)
    hh1 = tf.nn.dropout(h1, rate=drop_out)
    # 3rd hidden layer
    w2 = tf.get_variable('D_w2', [hh1.get_shape()[1], 256], initializer=w_init)
    b2 = tf.get_variable('D_b2', [256], initializer=b_init)
    h2 = tf.nn.leaky_relu(tf.matmul(hh1, w2) + b2)
    hh2 = tf.nn.dropout(h2, rate=drop_out)
    # output layer
    w3 = tf.get_variable('D_w3', [hh2.get_shape()[1], 1], initializer=w_init)
    b3 = tf.get_variable('D_b3', [1], initializer=b_init)
    o = tf.sigmoid(tf.matmul(hh2, w3) + b3)
...
def show_result(num_epoch, show = False, save = False, path = 'result.png'):
    ...
    output = sess.run(G_z, feed_dict={z: z_})
    fig = plt.figure(figsize=(5, 5))
    for i in range(25):
        fig.add_subplot(5, 5, i + 1).imshow(np.reshape(output[i, :], (28, 28)), cmap='gray')
        plt.axis('off')
    plt.savefig(path)
    plt.show()
